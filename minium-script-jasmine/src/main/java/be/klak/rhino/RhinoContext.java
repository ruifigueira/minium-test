...
public class RhinoContext {
    ...
    public void setProperty(String objectToReceiveProperty, String property, Object value) {
        ...
        if (obj == null || !(obj instanceof ScriptableObject)) {
            throw new IllegalStateException("object to receive property is no ScriptableObject but a " + (obj == null ? "" : obj.getClass().getSimpleName()));
        }
    ...
    // {{{ loadFromClasspath
    public void loadFromClasspath(String path, String ... resources) {
        if (!path.endsWith("/")) path = path + "/";
        if (path.startsWith("/")) path = path.substring(1);

        for (String resource : resources) {
            loadFromClasspath(path + resource);
        }
    }
     ...
     *
     * @param resource
     *            the resource to resolve from the classpath
     */
    public void loadFromClasspath(final String resource) {
        URL rsrcUrl = Thread.currentThread().getContextClassLoader().getResource(resource);
    ...
    public void loadEnv(String jsDir) {
        ...
        loadFromClasspath((jsDir.isEmpty() ? "" : "/") + "envJsOptions.js");
    }
